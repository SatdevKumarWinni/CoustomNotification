        ...
        val touchListener = object  : OnTouchListener{
            override fun onTouch(v: View?, event: MotionEvent?): Boolean {
                if (gestureDetector.onTouchEvent(event)) {
                    ...
                    else if(v?.id == view.tvMoreDetails.id){
...
//        view.rightLay.setOnClickListener { next() }
        ...
        view.tvMoreDetails.setOnTouchListener(touchListener)
    ...
    fun show() {
        view.loaderProgressbar.visibility = GONE
...
//            }, 2000)
    ...
    fun pause(withLoader : Boolean) {
        if(withLoader){
            view.loaderProgressbar.visibility = VISIBLE
        }
